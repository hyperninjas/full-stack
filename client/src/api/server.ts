 ...
 * ```
 */
export async function getServerApiConfig(options: ServerApiOptions = {}): Promise<Configuration> {
  const { tags, revalidate, cache, accessToken, authCookieName = 'auth_token' } = options;

  // Get auth token from cookies if not provided
  let token = accessToken;
  if (!token) {
    try {
      // Only try to read cookies in runtime contexts, not during static generation
      const cookieStore = await cookies();
      const authCookie = cookieStore.get(authCookieName);
      token = authCookie?.value;
    } catch (error) {
      // Cookies might not be available in all contexts (static generation, etc.)
      // This is expected and not an error - just continue without auth token
    }
  }
 ...
 * This is a re-export of Next.js's revalidateTag for convenience
 *
 * @example
 * ```ts
 * // In a Server Action
 ...
 * ```
 */
export { revalidateTag as revalidateApiCache } from 'next/cache';
